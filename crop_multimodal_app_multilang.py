...
# -*- coding: utf-8 -*-
"""
AgroSense Studio — Multilingual (EN/KN/TA/HI/ML/TE)
Tabular • Image • Text • Voice  |  Gradio 3.x-compatible UI

Run (example):
  python crop_multimodal_app_multilang.py \
...
# Minimal UI label translations (fallback to English if a key is missing)
UI = {
    "en": {
        "title": "🌿 AgriFusion — Multimodal AI Smart Farming Assistant",
        "inputs": "Inputs",
        "soil": "Soil Inputs",
    ...
    "hi": {
        "title": "🌿 AgriFusion — बहुभाषी स्मार्ट खेती सहायक",
        "inputs": "इनपुट",
        "soil": "मृदा इनपुट",
    ...
    "ta": {
        "title": "🌿 AgriFusion — பல்மொழி ச்மார்ட் விவசாய உதவியாளர்",
        "inputs": "உள்ளீடுகள்",
        "soil": "மண் தகவல்",
    ...
    "kn": {
        "title": "🌿 AgriFusion — ಬಹುಭಾಷಾ ಸ್ಮಾರ್ಟ್ ಕೃಷಿ ಸಹಾಯಕ",
        "inputs": "ಇನ್‌ಪುಟ್‌ಗಳು",
    ...
    "ml": {
        "title": "🌿 AgriFusion — ബഹുഭാഷാ സ്മാർട്ട് കൃഷി സഹായി",
        "inputs": "ഇൻപുട്ടുകൾ",
        ...
        "chat_send_voice": "വോയിസ് അയയ്ക്കുക",
        "chat_clear": "ചാറ്റ് ക്ലിയർ",
        "chat_tts": "TTS മറുപടി",
        "lang": "മറുപടി ഭാഷ",
    },
    "te": {
        "title": "🌿 AgriFusion — బహుభాషా స్మార్ట్ వ్యవసాయ సహాయకుడు",
        "inputs": "ఇన్‌పుట్లు",
...
# ======================= LLM / Whisper =======================
def ollama_chat(ollama_model: str, system: str, user: str, timeout=30) -> str:
    try:
        payload = {
            "model": ollama_model,
            "messages": [
...
def whisper_transcribe(audio_path: str, model_size="base"):
    ...
    try:
        ...
        # Default task=transcribe → keep original language;
        # you can force translation by task="translate".
...
def route_controller(tabular_inputs, image_path, text_q, voice_path, cfg, lang_code: str):
    ...
    def localize_prompt(prompt):
        """Force the model to reply in the chosen language."""
    ...
    if image_path:
        ...
        out["answer"] = ollama_chat(cfg["ollama_model"], system, user, timeout=cfg["llm_timeout"])
    ...
    if voice_path:
        ...
        out["answer"] = ollama_chat(cfg["ollama_model"], system, transcript, timeout=cfg["llm_timeout"])
        out["tts_path"] = _tts_if_possible(out["answer"], lang_code)
        out["context"] = {"mode": "voice", "prediction": "", "top": [], "recommendations": out["answer"]}
        return out
    ...
    if text_q and text_q.strip():
        ...
        out["answer"] = ollama_chat(cfg["ollama_model"], system, text_q, timeout=cfg["llm_timeout"])
        out["tts_path"] = _tts_if_possible(out["answer"], lang_code)
...
:root {
  --bg: #D0E8D0;       /* slightly darker green background */
  --card: #ffffff;      /* white card background */
  --accent: #145A1F;    /* darker rich green for buttons and highlights */
}
...
def build_interface(tabular_model_dir: Path, vision_model_dir: Path, images_dir: Path,
    ...
    with gr.Blocks(css=CUSTOM_CSS, title="AgroSense Studio — AI Crop Assistant") as demo:
        ...
        # For live relabeling when language changes
        ...
        gr.update(value="### " + ui_t(code, "image")),   # 🆕 image header
        gr.update(value="### " + ui_t(code, "results")), # 🆕 results header
    )
        ...
        # ---------- React to language changes (refresh a few labels) ----------
        lang_dd.change(
            _refresh_labels,
            inputs=[lang_dd],
            outputs=[
        ...
        def on_run(n,p,k,t,h,ph,r,image_path,text_q,voice_path,flags_str,ctx_str,keep_chat_flag,lang_choice):
            ...
            lang_code = lang_choice.split(" — ")[0].strip() if isinstance(lang_choice, str) else default_lang
...
def main():
    ...
    build_interface(Path(args.tabular_model_dir), Path(args.vision_model_dir), Path(args.images_dir),
                    args.ollama_model, args.port, args.whisper_size, args.llm_timeout, args.default_lang)
